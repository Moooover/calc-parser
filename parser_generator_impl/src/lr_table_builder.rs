...
impl LRTableEntryBuilder {
  pub fn lr_state(&self) -> &LRState {
    match self {
      LRTableEntryBuilder::Resolved(lr_state) => lr_state,
      LRTableEntryBuilder::Unresolved(_) => unreachable!(),
    }
  }
  ...
  pub fn state_builder_ref(&self) -> &LRStateBuilder {
    match self {
      LRTableEntryBuilder::Resolved(lr_state) => &lr_state.states,
      LRTableEntryBuilder::Unresolved(lr_state_builder) => lr_state_builder,
    }
  }

  pub fn as_parent_set(lr_entry_ptr: &Rc<LRTableEntryBuilder>) -> HashSet<Rc<LRTableEntryBuilder>> {
...
impl From<LRStateBuilder> for LRTableEntryBuilder {
  fn from(lr_state_builder: LRStateBuilder) -> Self {
    Self::Unresolved(lr_state_builder)
  }
}

impl Hash for LRTableEntryBuilder {
  fn hash<H: Hasher>(&self, state: &mut H) {
    self.state_builder_ref().hash(state);
  }
}

impl PartialEq for LRTableEntryBuilder {
  fn eq(&self, other: &Self) -> bool {
    self.state_builder_ref() == other.state_builder_ref()
  }
}

impl Eq for LRTableEntryBuilder {}

impl PartialOrd for LRTableEntryBuilder {
  fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
    self
      .state_builder_ref()
...
impl Ord for LRTableEntryBuilder {
  fn cmp(&self, other: &Self) -> std::cmp::Ordering {
    self.state_builder_ref().cmp(other.state_builder_ref())
  }
}

impl Display for LRTableEntryBuilder {
  fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
    match self {
      LRTableEntryBuilder::Resolved(lr_state) => write!(f, "{}", lr_state),
      LRTableEntryBuilder::Unresolved(lr_state_builder) => {
        write!(f, "unresolved({})", lr_state_builder)
      }
    }
  }
}
    ...
    match self {
      Action::Shift(lr_table_entry) => write!(
        ...
        match lr_table_entry.deref() {
          LRTableEntryBuilder::Resolved(lr_state) => {
            lr_state
              .states
              .states
              .iter()
...
pub struct TransitionSet {
  ...
  pub goto_map: BTreeMap<ProductionRef, Rc<LRTableEntryBuilder>>,
}
...
#[derive(Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct LRState {
  pub states: Transparent<LRStateBuilder>,
  pub transitions: Transparent<TransitionSet>,
  pub parent_states: Transparent<HashSet<Rc<LRTableEntryBuilder>>>,
  pub uid: u64,
}
...
impl LRState {
  pub fn new<I: Iterator<Item = ProductionState>>(
    ...
    parent_states: HashSet<Rc<LRTableEntryBuilder>>,
    uid: u64,
  ) -> Self {
    Self {
      ...
      parent_states: parent_states.into(),
      uid,
    }
  }

  fn add_parent(&mut self, parent: Rc<LRTableEntryBuilder>) {
    self.parent_states.insert(parent);
  }

  pub fn parents_of(
    lr_states: &HashSet<Rc<LRTableEntryBuilder>>,
  ) -> HashSet<Rc<LRTableEntryBuilder>> {
    lr_states
      .iter()
      .fold(HashSet::new(), |mut parent_states, lr_state| {
        parent_states.extend(lr_state.lr_state().parent_states.clone().data());
        parent_states
      })
  }
  ...
  pub fn merge(&mut self, other: LRState) {
    self.states.merge(other.states.data());
    self.parent_states.extend(other.parent_states.data());
  }
...
struct StateMap {
  map: HashMap<Rc<LRTableEntryBuilder>, Rc<LRTableEntryBuilder>>,
}
...
impl StateMap {
  ...
  fn insert(&mut self, from: Rc<LRTableEntryBuilder>, to: Rc<LRTableEntryBuilder>) {
    self.map.insert(from, to);
  }

  fn remap(&mut self, mut entry: Rc<LRTableEntryBuilder>) -> Rc<LRTableEntryBuilder> {
...
pub struct LRTable {
  pub states: HashSet<Rc<LRTableEntryBuilder>>,
  pub initial_state: Rc<LRTableEntryBuilder>,
}

impl LRTable {
  fn calculate_transitions(
    states: &mut HashSet<Rc<LRTableEntryBuilder>>,
    grammar: &Grammar,
    first_table: &mut ProductionFirstTable,
    lr_state_builder: &LRStateBuilder,
    parent_lr_state: Option<Rc<LRTableEntryBuilder>>,
    next_uid: &mut u64,
  ) -> ParseResult<Rc<LRTableEntryBuilder>> {
    let lr_table_entry = LRTableEntryBuilder::from(lr_state_builder.clone());
    ...
    if let LRTableEntryBuilder::Unresolved(lr_state_builder) = lr_state.deref() {
      let mut parent_states = HashSet::new();
      if let Some(parent_state) = parent_lr_state {
        parent_states.insert(parent_state);
      }
      unsafe {
        *Rc::get_mut_unchecked(&mut lr_state) = LRTableEntryBuilder::Resolved(LRState::new(
  ...
  fn merge_redundant_states(&mut self) {
    loop {
      let mut transition_set_map: HashMap<TransitionSet2, Rc<LRTableEntryBuilder>> = HashMap::new();
      let mut remapped_states = StateMap::new();

      self.states.iter().for_each(|lr_table_entry| {
        let transition_set: TransitionSet2 =
          lr_table_entry.lr_state().transitions.clone().data().into();
